...
$dir="http://irdp.ncl.res.in/out/".$rand;
...
echo("<br><br>");
help("help_imotifs.html#results");	
printSummary("/out/$rand/$subpref1/Pattern_summary.html");
...
Cufon.now();
function callJmol(doc, molName, molFileName, motifChain, motifStart, motifLength, prefix)
	{
		var JmolSize = 520
		var JmolWinWidth = JmolSize+10
		var JmolWinHeight = JmolSize+54
		var callURL = "/showModel.html?name=" + escape(molName) + "&file=" + escape(molFileName)+ "&start=" + escape(motifStart)+ "&length=" + escape(motifLength)+"&chain=" + escape(motifChain)+"&prefix=" + escape(prefix)
		
		var op 
		var winName = ""
		winName = "Jmol_structure"
		
		if (!Jmol_struct || Jmol_struct.closed) {
			Jmol_struct = window.open( callURL, winName, "width=" + JmolWinWidth + ", height=" + JmolWinHeight + ", resizable" );
		}
		else {
			Jmol_struct.updateMotif(motifStart, motifLength, motifChain);
		}
		Jmol_struct.focus(); 
